...
class File_Splitter(object):
    ...
    def copy_file(self, rhs, ignore_header=True):
        ...
        return (rhs, total_lines, self._header_line)
    ...
    def file_size():
        """
        DOS files have a dopey \r\n end of line. Python silently parses this but it means
        comparison between size files generated by this program and dos source files is problematic.
        To get a size that compares with the split files. Just pass int he :param filename and the :param no_of_lines
        and this will do the rest.
        :param filename: The file we want to calculate the size for
        :param no_of_lines: the number of lines in the file
        :return: total size minus the \r characters
        """

        return os.path.getsize( self._input_filename ) - self._
