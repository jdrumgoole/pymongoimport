...
class Test(unittest.TestCase):
    ...
    def test_FieldConfig(self):
        fc = FieldFile(None, f("data/test_fieldconfig.ff"))
        cfg = fc.config()
        ...
        fc = FieldFile(None, f("data/uk_property_prices.ff"))
        cfg = fc.config()
    ...
    def test_delimiter_no_header(self):
        ...
        fc = FieldFile(None, f("data/10k.ff"), delimiter='|', hasheader=False)
    ...
    def test_delimiter_header(self):
        start_count = self._col.count_documents({})
        fc = FieldFile(None, f("data/AandE_Data_2011-04-10.ff"), delimiter=',', hasheader=True)
        bw = File_Writer(self._col, fc)
    ...
    def test_generate_field_filename(self):
        fc_filename = FieldFile.generate_field_filename(f('data/inventory.csv'))
        self.assertEqual(fc_filename, f("data/inventory.ff"), fc_filename)
        fc_filename = FieldFile.generate_field_filename(f('data/inventory.csv'), ext="xx")
        ...
        fc_filename = FieldFile.generate_field_filename(f('data/inventory.csv.1'))
        self.assertEqual(fc_filename, f("data/inventory.ff"), fc_filename)

        fc_filename = FieldFile.generate_field_filename(f('data/yellow_tripdata_2015-01-06-200k.csv.1'))
        self.assertEqual(fc_filename, f("data/yellow_tripdata_2015-01-06-200k.ff"), fc_filename)

        fc_filename = FieldFile.generate_field_filename(f('data/yellow_tripdata_2015-01-06-200k.csv.10'))
        self.assertEqual(fc_filename, f("data/yellow_tripdata_2015-01-06-200k.ff"), fc_filename)

        fc_filename = FieldFile.generate_field_filename(f('test_result_2016.txt.1'))
        self.assertEqual(fc_filename, f("test_result_2016.ff"), fc_filename)

    def test_dict_reader(self):
        fc_filename = FieldFile.generate_field_file(f("data/inventory.csv"))
        fc = FieldFile(None, fc_filename)
        cfg = fc.config()
    ...
    def test_generate_fieldfile(self):
        fc_filename = FieldFile.generate_field_file(f("data/inventory.csv"), ext="testff")
        self.assertTrue(os.path.isfile(f("data/inventory.testff")))
        fc = FieldFile(None, fc_filename, hasheader=True)
        config = fc.config()
        start_count = self._col.count_documents({})
    ...
    def test_date(self):
        fc = FieldFile(None, f("data/inventory_dates.ff"), hasheader=True)
        config = fc.config()
        start_count = self._col.count_documents({})
        writer = File_Writer(self._col, fc)
        writer.insert_file(f("data/inventory.csv"))
    ...
    def test_field_config_exception(self):
        ...
        self.assertRaises(OSError, FieldFile, None, "nosuchfile.ff")
        # print( "fields: %s" % f.fields())

    def testFieldDict(self):
        fc = FieldFile(None, f("data/testresults.ff"), delimiter="|")
    ...
    def test_duplicate_id(self):
        self.assertRaises(ValueError, FieldFile, None, f("data/duplicate_id.ff"))
